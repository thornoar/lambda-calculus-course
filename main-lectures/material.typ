...
#agr("правило переменных")[
  Пусть л-выражения $M_1, M_2, ..., M_n$ выступают с едином контексте. Тогда мы будем предполагать, что все комбинации этих выражений корректны. Более того, с данного момента мы будем факторизовать множество $#L$ по отношению л-конгруэнтности, то есть $M == N <=> M =^alpha N$.
]
