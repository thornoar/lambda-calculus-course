...
+ Рассмотрим последовательность $a_n == #K^n #I$. Покажите, что $a$ --- *не* числовая система.
  #hint[
    ...
    Для фиксированного $n in NN_0$, рассмотрите крайний левый редукционный путь
    $
      Z a_n equiv P_0 -->_beta P_1 -->_beta ... -->_beta P_N equiv H,
    $
    #v(-2.5mm)
    где $H$ обозначает $K$ или $K^*$. Попытайтесь "выделить" выражение $a_n$ из $P_i$, для $0 <= i <= N$.
    // Докажите, что для всех $i in {1,2,...,N}$ выражение $P_i$ _за один шаг_ редуцируется путём
    // #v(-2mm)
    // $
...
+ Пусть $M_1 == (@x.. b x (b c))c$, $M_2 == (@x.. x x)(b c)$. Докажите, что *не* существует такого выражения $A$, что $A ->> M_1$ и $A ->> M_2$.
  #hint[
    ...
    $
    #v(-2.5mm)
    часть $[...]$ слева от $(@x.. P)Q$ остаётся неизменной при последующих редукциях в редукционном пути $M ->> N$. Это можно формализовать так: пусть запись $A -->^n B$ означает, что $B$ получается из $A$ путём редукции $n$-ного по счёту слева редекса в выражении $A$. Тогда редукционный путь
    #v(-2.5mm)
    $
