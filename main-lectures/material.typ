...
#exer[
  Доказать, что если $M$ корректно, то
  $
    FV(M) inter BV(M) = empty, #h(2em) FV(M) union BV(M) = TV(M).
  $
]

#exer[
  Пусть $M$ --- л-выражение. Доказать, что существует такое корректное л-выражение $N$, что $M acongr N$.
]
...
#pf[
  ...
  + Переход.
    - $M equiv @z.. M'$. По правилу переменных и определению оператора подстановки мы имеем $z in.not FV(N L)$ и $z equiv.not x,y$. Тогда по предположению индукции
    $
...
#pf[
  ...
  + Пусть $N_1, N_2$ --- б-нормальные формы выражения $M$. Тогда $N_1 ->>_beta Z$ и $N_2 ->>_beta Z$ для некоторого $Z$. Следовательно, $N_1 == Z == N_2$.
  q.e.d.
]
...
#def[
  Пусть $rel1, rel2, rel3, rel4$ --- бинарные отношения на множестве $X$. Следующая диаграмма,
  #diamond-diagram(
    spacing: 2cm,
    $rel1$,
    $rel2$,
...
#def("редукционная стратегия")[
  Отображение $F : #L -> #L$ называется _редукционной стратегией,_ если для любого $M in #L$ выполняется редукция
  // #v(-5pt)
  $
    M ->> F(M).
  $
...
#def[
  + Пусть $F$ --- редукционная стратегия. _$F$-редукционный путь_ выражения $M$ --- это последовательность
    ...
    $
      M,, F(M),, F^2(M),, ...
    $
  + $F$ называется _нормализующей,_ если для любого $M in #L$, имеющего нормальную форму, $F^n (M)$ находится в нормальной форме для некоторого $n in NN$.
]
...
#def[
  ...
  + Пусть $B in #L$. Тогда запись #h(1fr)
    #v(-5pt)
    $
      ite(B, M, N)
    $
...
#def[
  Пусть $M,N in #L$. _Упорядоченная пара_ $[M, N]$ определяется как
  $
    [M, N] == @z.. z M N.
  $
...
#pf[
  Определим выражения
  $
    M &== @f%x.. f (#P^n_1 x) (#P^n_2 x) ... (#P^n_n x),\
    F &== @x.. <<M F_1 x, M F_2 x, ..., M F_n x>>.
  $
  Тогда по теореме о неподвижной точке найдётся выражение $X in #L: hs X = F X$. Наконец, положим $X_i == #P^n_i X$. Действительно,
  $
    X_i == #P^n_i X = M F_i X = F_i X_1 X_2 ... X_n,
  $
  q.e.d.
]
...
#def[
  ...
  + Определим
    $
      Sp == @z.. [#F, z], #h(1cm) Pm == @z.. z#F, #h(1cm) Zero == @z.. z#T.
    $
]
...
#def[
  ...
  $
    mu m [ P(m) ]
  $
обозначает наименьшее число $m$, такое, что выполняется $P(m)$, если такое число существует. В противном случае $mu m [ P(m) ]$ не определено.
]
...
#def[
  ...
  + #A называется _замкнутым относительно суперпозиции,_ если для любых $chi, psi_1, psi_2, ..., psi_m in #A$, функция 
    ...
    $
    лежит в #A.
  + #A называется _замкнутым относительно примитивной рекурсии,_ если для любых $chi, psi in #A$, функция
    $
...
#lm[
  ...
  $
    phi (arrow(n)) = m hs <=> hs F num(arrow(n)) = num(m)
  $
]
#pf[
  / ($==>$): Очевидно по определению.
...
#state[
  Существуют л-выражения $H$, $H^(-1)$, такие, что при всех $n in NN_0$
  $
...
#lm[
  ...
  #v(-5pt)
  $
    Ap c_n c_m = c_(n+m), #h(1cm) Am c_n c_m = c_(n m), #h(1cm) Ae c_n c_m = c_(n^m).
  $
  #v(-5pt)
]
#pf[
...
#note[
  Числа Чёрча хороши тем, что на них очень простая арифметика. Однако они плохи отсутствием естественного предшествующего элемента.
]
...
#def[
  Пусть $#A subset #L$. Тогда $#A$ называется _замкнутым относительно конверсии,_ если для любых $M, N in #L$:
  $
    cases(
      reverse: #true,
      M in #A,
